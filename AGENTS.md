...
- 当前 Kylin 本机已运行 Mihomo：`mihomo.service`，配置文件 `/home/cmcc/.config/mihomo/config.yaml`，mixed proxy 端口 `http://127.0.0.1:7890`。访问 GitHub、OpenAI、海外文档或其他外网资源时，直连不通要显式使用该代理，例如 `curl -x http://127.0.0.1:7890 ...` 或 `git -c http.proxy=http://127.0.0.1:7890 fetch ...`。
- GitHub 推送认证：当前 Kylin 的 `/home/cmcc/.ssh/` 里没有 GitHub 推送私钥，只有 `authorized_keys` 和 `known_hosts`；不要假设 `git@github.com` 可直接推送。最近一次推送是从 Windows 本机通过 `gh` 的 HTTPS 登录态完成，令牌保存在 Windows GitHub CLI/系统凭据中，不在仓库文件里，不能输出或提交任何 token/private key。若要让 Kylin 自己推送，需要在 `/home/cmcc/.ssh/` 下单独放置 GitHub deploy key/私钥并配置 Git/SSH 走 `127.0.0.1:7890` 代理。
